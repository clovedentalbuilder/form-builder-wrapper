 ...
 * validation UI) so its validators never touch the fx library's shared
 * FxValidatorService statics — sidesteps the cross-field validation-message
 * bleed that native fx-text-field is prone to.
 *
 * Settings (own dialog, same chrome pattern as Section/Repeatable Group):
 *   - Basic: label, placeholder, help text
 *   - Validation: required / minLength / maxLength / pattern, each with its
 *     own error message, enforced via plain Angular Validators
 *   - Enable/Disable & Visibility: each a condition list — rows freely mixing
 *     privilege / supportingData / another-field's-value sources, combined
 *     via per-condition AND/OR grouping — OR custom JS code. See
 *     shared/applicability.ts.
 *
 * Hidden fields always keep their control registered and value included in
 * the form — no "exclude from form when hidden" option; only visual
 * visibility is gated.
 */
@Component({
  selector: 'lib-custom-textbox',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, CustomTextboxSettingsPanelComponent],
  templateUrl: './custom-textbox.component.html',
  styleUrl: './custom-textbox.component.css',
})
export class CustomTextboxComponent extends FxBaseComponent {
