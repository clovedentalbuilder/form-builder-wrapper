...
/**
 * Custom settings panel for the custom Textarea field (same chrome pattern as
 * lib-custom-textbox). Three tabs — Basic Config (display/label/visibility/
 * enable), Validations, and Import (JSON import, mirroring Dropdown with
 * Child Field's Quick Setup; export lives in the footer).
 *
 ...
 * fields always keep their value in the form now (see CustomTextareaComponent).
 *
 ...
 */
@Component({
  selector: 'lib-custom-textarea-settings-panel',
  standalone: true,
  imports: [CommonModule, DialogModule, ButtonModule, InputTextModule, FormsModule, ReactiveFormsModule],
  templateUrl: './custom-textarea-settings-panel.component.html',
  styleUrl: './custom-textarea-settings-panel.component.css',
  encapsulation: ViewEncapsulation.None,
})
export class CustomTextareaSettingsPanelComponent extends FxComponent {
  ...
  exportCurrentConfig(): void {
    ...
    a.href = url;
    a.download = `custom-textarea-${raw.name || 'export'}.json`;
    a.click();
